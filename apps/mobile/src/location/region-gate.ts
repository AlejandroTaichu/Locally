...
export type RegionGateResult = { allowed: true } | { allowed: false; reason: 'permission-denied' | 'unsupported-region' };
...
function isWithinSupportedCity(lat: number, lng: number): boolean {
  return SUPPORTED_CITIES.some((city) => haversineDistanceKm(lat, lng, city.lat, city.lng) <= city.radiusKm);
}

export async function checkRegionAccess(): Promise<RegionGateResult> {
  ...
  try {
    ...
    if (isWithinSupportedCity(position.coords.latitude, position.coords.longitude)) {
      return { allowed: true };
    }
